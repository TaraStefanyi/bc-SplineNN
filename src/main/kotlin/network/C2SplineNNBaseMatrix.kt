...
import mapColsIndexed
import mapRowsToListIndexed
...
class C2SplineNNBaseMatrix(
        ...
        val splineInitFunction: ActivationFunction = SimpleActivationFunction.TANH,
...
//        val baseMatrixes= listOf(network.SplineType.CATMULROM.baseMatrix)
        val v = if (u.count() > 1) {
...
fun main() {
    ...
    val f = network.splineInitFunction::invoke
    val d = network.splineInitFunction::derivative
    val m = dot(mat[-0.51 pow 3, -0.51 pow 2, -0.51, 1] * network.baseMatrixForAB(-1.0, 0.0), mat[f(-1.0), f(0.0), d(-1.0), d(0.0)])
//    val m = dot(mat[-0.51 pow 3, -0.51 pow 2, -0.51, 1] * network.baseMatrixForAB(-0.6, -0.5), mat[f(-0.6), f(-0.5), d(-0.6), d(-0.5)])
    println(m)
}